...
# 
# Gman[[8]] <-  bind_rows(Gman[[8]] ,Gman[[11]]) %>% group_by(name) %>% summarise(tv = last(tv), race = first(race), logo = first(logo), name.1 = first(name.1), score = sum(score), id = first(id)) %>% arrange(desc(score))
# 
...
# 
# REL[[8]] <- bind_rows(REL[[8]] ,REL[[14]]) %>% group_by(name) %>% summarise(tv = last(tv), race = first(race), logo = first(logo), name.1 = first(name.1), score = sum(score), id = first(id)) %>% arrange(desc(score))
# 
...
data <- api_contests(key, league = "REBBL Playoffs", competition = "REBBL Playoffs - Season 8", limit = 100, status = "played")
